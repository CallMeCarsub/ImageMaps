...
public class InventoryUtils {
     ...
     */
    public static boolean containsAtLeast(Inventory inv, ItemStack... itemStacks) {
     ...
     * @return true when there are enough items in the inventory, false otherwise
     */
    public static boolean containsAtLeast(Inventory inv, Material material, int amount) {
     ...
     */
    public static int remove(Inventory inv, Material material, int amount) {
        if(amount <= 0)
            return 0;
        for (ItemStack item : inv) {
            if (item.getType() != material)
    ...
    public static ItemStack[] deepCopyItemStacks(ItemStack[] array) {
